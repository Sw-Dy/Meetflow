...
fn main() {
    ...
    if env::var("WHISPER_DONT_GENERATE_BINDINGS").is_ok() || target.contains("windows") {
        let mut bindings = std::fs::read_to_string("src/bindings.rs")
            .expect("Failed to read bundled bindings.rs");

        if target.contains("windows") {
            bindings = remove_layout_test(bindings, r#"["Size of _G_fpos_t"]"#);
            bindings = remove_layout_test(bindings, r#"["Size of _G_fpos64_t"]"#);
            bindings = remove_layout_test(bindings, r#"["Size of _IO_FILE"]"#);
        ...
        std::fs::write(out.join("bindings.rs"), bindings).expect("Failed to write bindings.rs");
    } else {
...
fn remove_layout_test(mut bindings: String, marker: &str) -> String {
    let Some(marker_pos) = bindings.find(marker) else {
        return bindings;
    };

    let Some(block_start) = bindings[..marker_pos].rfind("#[allow(clippy::unnecessary_operation, clippy::identity_op)]") else {
        return bindings;
    };

    let Some(relative_end) = bindings[marker_pos..].find("\n};") else {
        return bindings;
    };

    let block_end = marker_pos + relative_end + "\n};".len();
    bindings.replace_range(block_start..block_end, "");
    bindings
}
