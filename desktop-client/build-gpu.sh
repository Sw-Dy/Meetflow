#!/bin/bash
# GPU-accelerated build script for MeetFlow
...
echo -e "${BLUE}🚀 MeetFlow GPU-Accelerated Build Script${NC}"
echo ""
...
if [ -f "package.json" ]; then
  ...
  cd desktop-client || {
    echo -e "${RED}❌ Failed to change to desktop-client directory${NC}"
    exit 1
  }
  FRONTEND_DIR="."
else
...
echo ""
echo -e "${BLUE}📦 Building MeetFlow...${NC}"
echo ""
...
if [ -z "$TAURI_GPU_FEATURE" ]; then
    ...
    # We need to run it from desktop-client dir
