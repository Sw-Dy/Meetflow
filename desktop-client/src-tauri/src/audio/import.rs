...
use crate::audio::decoder::{decode_audio_file, decode_audio_file_to_whisper_format_with_progress};
use crate::audio::vad::get_speech_chunks_with_progress;
use crate::config::{DEFAULT_PARAKEET_MODEL, DEFAULT_WHISPER_MODEL};
use crate::parakeet_engine::ParakeetEngine;
use crate::state::AppState;
use crate::whisper_engine::{ModelStatus, WhisperEngine};
...
/// Default model for imported files when the dialog does not explicitly choose one.
/// CPU-only Windows builds are too slow with the global large-v3-turbo default.
const FAST_IMPORT_WHISPER_MODEL: &str = "base-q5_1";
...
pub async fn start_import<R: Runtime>(
    ...
    let use_parakeet =
        provider.as_deref() == Some("parakeet") && language.as_deref() != Some("auto-translate");
...
async fn run_import<R: Runtime>(
    ...
    // Determine which provider to use (default to whisper). Translation to English requires Whisper.
    let use_parakeet =
        provider.as_deref() == Some("parakeet") && language.as_deref() != Some("auto-translate");
    ...
    emit_progress(&app, "decoding", 15, "Preparing fast audio conversion...");

    // Decode, mix to mono, and resample to 16kHz in one FFmpeg-backed pass.
    // The old Rust decode + sinc resample path remains available as fallback
    // inside the decoder, but the fast path avoids long stalls at 20%.
    let app_for_decode = app.clone();
    let decode_progress = Box::new(move |progress: u32, msg: &str| {
        // Map conversion progress: 15% + (progress * 0.10) to go from 15% to 25%
    ...
    let decoded = tokio::task::spawn_blocking(move || {
        decode_audio_file_to_whisper_format_with_progress(&path_for_decode, Some(decode_progress))
    })
    .await
    ...
    info!(
        "Converted audio: {:.2}s, {}Hz, {} channels",
    ...
    let whisper_model = if provider.as_deref() == Some("parakeet") {
        None
    } else {
        model.as_deref()
    };
    let whisper_engine = if !use_parakeet && total_segments > 0 {
        Some(get_or_init_whisper(&app, whisper_model).await?)
    } else {
        None
    };
    ...
    match engine {
        Some(e) => {
            let target_model = match requested_model {
                ...
                None => select_fast_import_whisper_model(app, &e).await?,
            };
...
async fn select_fast_import_whisper_model<R: Runtime>(
    app: &AppHandle<R>,
    ...
    let models = engine
        .discover_models()
        .await
        .map_err(|e| anyhow!("Failed to discover Whisper models: {}", e))?;

    let preferred = [
        FAST_IMPORT_WHISPER_MODEL,
        "tiny-q5_1",
        "base",
        "tiny",
        "small-q5_1",
    ...
    for model_name in preferred {
        ...
        {
            info!("Using fast import Whisper model '{}'", model_name);
            return Ok(model_name.to_string());
        }
    }

    let configured = get_configured_model(app, "whisper").await?;
    if models
        .iter()
        .any(|model| model.name == configured && matches!(model.status, ModelStatus::Available))
    {
        info!("Using configured import Whisper model '{}'", configured);
    ...
    models
        ...
        .ok_or_else(|| {
            anyhow!(
                "No downloaded Whisper model is available. Download base-q5_1 for fast imports."
            )
        })
}
...
mod tests {
    ...
    fn test_extract_duration_from_metadata_wav() {
        ...
        let test_path = Path::new("../../legacy-service/whisper.cpp/samples/jfk.wav");
        if test_path.exists() {
    ...
    fn test_extract_duration_from_metadata_mp3() {
        ...
        let test_path = Path::new("../../legacy-service/whisper.cpp/samples/jfk.mp3");
        if test_path.exists() {
            let result = extract_duration_from_metadata(test_path);
