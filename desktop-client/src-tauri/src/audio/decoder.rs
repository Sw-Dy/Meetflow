...
/// Decode directly to Whisper-compatible 16kHz mono f32 samples via FFmpeg.
///
/// This is the fast import path. It avoids the expensive two-pass
/// decode-then-resample flow and lets FFmpeg do decode, mono mixdown, and
/// sample-rate conversion in one native streaming pass.
pub fn decode_audio_file_to_whisper_format_with_progress(
    path: &Path,
    progress_callback: Option<ProgressCallback>,
) -> Result<DecodedAudio> {
    match decode_audio_file_to_whisper_format_with_ffmpeg(path, progress_callback) {
        Ok(decoded) => Ok(decoded),
        Err(err) => {
            warn!(
                "Fast FFmpeg import path failed for {}: {}. Falling back to Rust decoder.",
                path.display(),
                err
            );
            let decoded = decode_audio_file_with_progress(path, None)?;
...
fn decode_audio_file_to_whisper_format_with_ffmpeg(
    input_path: &Path,
    progress_callback: Option<ProgressCallback>,
) -> Result<DecodedAudio> {
    const WHISPER_SAMPLE_RATE: u32 = 16000;
    ...
    let ffmpeg_path =
        find_ffmpeg_path().ok_or_else(|| anyhow!("FFmpeg not found for fast audio import"))?;
    let input_str = input_path
        .to_str()
        .ok_or_else(|| anyhow!("Invalid input path (non-UTF8)"))?;

    if let Some(cb) = &progress_callback {
        cb(0, "Starting fast audio conversion...");
    }
    ...
    debug!("Fast FFmpeg import command: {:?}", command);
    ...
    let mut last_reported_seconds = 0usize;

    loop {
        let read = stdout
            .read(&mut buffer)
            .map_err(|e| anyhow!("Failed reading ffmpeg audio output: {}", e))?;
        if read == 0 {
            break;
        }
        ...
        if let Some(cb) = &progress_callback {
            let converted_seconds =
                raw.len() / (WHISPER_SAMPLE_RATE as usize * std::mem::size_of::<f32>());
            if converted_seconds >= last_reported_seconds + 10 {
                last_reported_seconds = converted_seconds;
                cb(
                    50,
                    &format!(
                        "Converted {:.1} minutes of audio...",
                        converted_seconds as f64 / 60.0
                    ),
                );
            }
        }
    ...
    if !status.success() {
        error!(
            "Fast FFmpeg import failed (exit code: {}): {}",
            status, stderr_text
        );
        return Err(anyhow!(
            "FFmpeg conversion failed with exit code: {}. The file may be corrupted or unsupported.",
            status
        ));
    }

    if raw.len() < std::mem::size_of::<f32>() {
        return Err(anyhow!("FFmpeg produced no audio samples"));
    }

    let sample_count = raw.len() / std::mem::size_of::<f32>();
    ...
    if let Some(cb) = &progress_callback {
        cb(100, "Fast audio conversion complete");
    }

    info!(
        "Fast FFmpeg import complete: {} samples ({:.2}s) at {}Hz mono",
        samples.len(),
        duration_seconds,
        WHISPER_SAMPLE_RATE
    );
