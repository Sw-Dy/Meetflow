...
fn english_translation_mode_enabled() -> bool {
    matches!(
...
pub async fn validate_transcription_model_ready<R: Runtime>(
    ...
    let force_whisper_for_translation = english_translation_mode_enabled();

    // Validate based on provider
    match (force_whisper_for_translation, config.provider.as_str()) {
        (true, _) | (_, "localWhisper") => {
            if force_whisper_for_translation && config.provider != "localWhisper" {
                info!(
                    "English translation mode is active; using Whisper instead of configured provider '{}'",
                    config.provider
                );
            }
...
pub async fn get_or_init_transcription_engine<R: Runtime>(
    ...
    match (force_whisper_for_translation, config.provider.as_str()) {
        ...
        (true, _) | (_, "localWhisper") | (_, _) => {
            if force_whisper_for_translation && config.provider != "localWhisper" {
                info!(
                    "English translation mode is active; routing '{}' provider through Whisper",
                    config.provider
                );
            }
    ...
    .await
    {
        Ok(Some(config)) => {
            ...
            if config.provider == "localWhisper" {
                ...
                config.model
            } else if english_translation_mode_enabled() {
                info!(
                    "English translation mode is active; using default Whisper model instead of provider '{}'",
                    config.provider
                );
        ...
        Some(model) => {
            match model.status {
                ...
                crate::whisper_engine::ModelStatus::Missing => {
                    if english_translation_mode_enabled() {
                        ...
                        if let Some(fallback_model) = available_models.first() {
                            warn!(
                                "Default translation model '{}' is not downloaded, falling back to available Whisper model '{}'",
                                model_to_load, fallback_model.name
                            );
                            engine.load_model(&fallback_model.name).await.map_err(|e| {
                                format!(
                                    "Failed to load fallback translation model '{}': {}",
                                    fallback_model.name, e
                                )
                            })?;
                            info!(
                                "âœ… Fallback translation model '{}' loaded successfully",
                                fallback_model.name
                            );
                            return Ok(engine);
                        }
                    ...
                    return Err(format!(
                        "Model '{}' is not downloaded. Please download a Whisper model from Transcript Settings to translate Bengali/Hindi speech into English.",
                        model_to_load
                    ));
                }
