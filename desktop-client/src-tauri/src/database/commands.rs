...
/// Detect legacy database from a selected path (root repo, legacy-service folder, or db file)
#[tauri::command]
pub async fn detect_legacy_database(selected_path: String) -> Result<Option<String>, String> {
    ...
    if path.is_dir() {
        ...
        // Case 3: User selected root repo (check legacy-service subdirectory)
        let backend_db = path.join("legacy-service").join("meeting_minutes.db");
        if backend_db.exists() && backend_db.is_file() {
            let db_path = backend_db.to_string_lossy().to_string();
            info!("Found database in legacy-service subdirectory: {}", db_path);
            return Ok(Some(db_path));
        }
    }
...
/// This is specifically for detecting old Python legacy-service installations
#[tauri::command]
