...
fn configure_onnx_runtime_path<R: tauri::Runtime>(app: &tauri::AppHandle<R>) {
    #[cfg(target_os = "windows")]
    {
        let candidates = [
            app.path()
                .resource_dir()
                .ok()
                .map(|dir| dir.join("binaries").join("onnxruntime.dll")),
            std::env::current_exe()
                .ok()
        ...
        for candidate in candidates.into_iter().flatten() {
            if candidate.is_file() {
                ...
                log::info!("Using ONNX Runtime DLL: {}", candidate.display());
                return;
            }
        }

        log::warn!("ONNX Runtime DLL not found; VAD may load an incompatible system DLL");
    }
}
...
pub fn run() {
    ...
    builder
        ...
        .setup(|_app| {
            ...
            configure_onnx_runtime_path(&_app.handle());
