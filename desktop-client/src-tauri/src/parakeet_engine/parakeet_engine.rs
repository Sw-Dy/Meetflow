    ...
    pub async fn discover_models(&self) -> Result<Vec<ModelInfo>> {
        ...
        let model_configs = [
            (
                ...
                "Fast (v2)",
                "Fast local transcription with int8 quantization, good balance of speed and accuracy",
            ),
        ];
    ...
    pub async fn download_model_detailed(
        ...
        let base_url = match model_name {
            ...
            _ => {
                ...
                return Err(anyhow!(
                    "Unsupported Parakeet model '{}'. Use parakeet-tdt-0.6b-v2-int8.",
                    model_name
                ));
            }
        };
        ...
        for (index, filename) in files_to_download.iter().enumerate() {
            ...
            let mut request = client
                .get(&file_url)
                .header(reqwest::header::USER_AGENT, "MeetFlow/0.4 model downloader")
                ...
                if response.status() == reqwest::StatusCode::PARTIAL_CONTENT {
                    ...
                    if existing_size >= size_tolerance && expected_size > 0 {
                        ...
                        if !response.status().is_success() {
                            ...
                            return Err(anyhow!(
                                "Retry failed for {} from {} with status: {}",
                                filename,
                                file_url,
                                response.status()
                            ));
                    ...
                    return Err(anyhow!(
                        "Download failed for {} from {} with status: {}",
                        filename,
                        file_url,
                        response.status()
                    ));
