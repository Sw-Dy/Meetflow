...
impl WhisperEngine {
    ...
    pub fn new_with_models_dir(models_dir: Option<PathBuf>) -> Result<Self> {
        ...
        let models_dir = if let Some(dir) = models_dir {
            ...
            // Development: Use desktop-client/models or legacy-service directories
            // Production: Use system directories (should be overridden by caller)
            if cfg!(debug_assertions) {
                // Development mode - try desktop-client and legacy-service directories
                if current_dir.join("models").exists() {
                    current_dir.join("models")
                } else if current_dir.join("../models").exists() {
                    current_dir.join("../models")
                } else if current_dir
                    .join("legacy-service/whisper-server-package/models")
                    .exists()
                {
                    current_dir.join("legacy-service/whisper-server-package/models")
                } else if current_dir
                    .join("../legacy-service/whisper-server-package/models")
                    .exists()
                {
    ...
    pub async fn transcribe_audio_with_confidence(
        ...
        let current_model = self.current_model.read().await.clone().unwrap_or_default();
        let sampling_strategy =
            if current_model.starts_with("tiny") || current_model.starts_with("base") {
