...
export function OnboardingFlow({ onComplete }: OnboardingFlowProps) {
  ...
  // Step 1: Welcome - Introduce MeetFlow features
  ...
  const visibleStep = currentStep === 4 && !isMac ? 3 : currentStep;

  return (
    <div className="onboarding-flow">
      {visibleStep === 1 && <WelcomeStep />}
      {visibleStep === 2 && <SetupOverviewStep />}
      {visibleStep === 3 && <DownloadProgressStep />}
      {visibleStep === 4 && isMac && <PermissionsStep />}
    </div>
  );
}
