...
export function DownloadProgressStep() {
  ...
  const handleRetryDownload = async () => {
    if (!isTauriRuntime()) {
      toast.error('Desktop runtime required', {
        description: 'Open MeetFlow through Tauri to download the transcription engine.',
      });
      return;
    }
  ...
  const handleRetrySummaryDownload = async () => {
    if (!isTauriRuntime()) {
      toast.error('Desktop runtime required', {
        description: 'Open MeetFlow through Tauri to download the summary engine.',
      });
      return;
    }
  ...
  useEffect(() => {
    setHasDesktopRuntime(isTauriRuntime());
  ...
  useEffect(() => {
    if (!hasDesktopRuntime) {
      setParakeetState((prev) => ({
        ...prev,
        status: 'error',
        error: 'Desktop runtime is not available. Run the Tauri app to download this model.',
      }));
      setSummaryState((prev) => ({
        ...prev,
        status: 'waiting',
  ...
  }, [hasDesktopRuntime, parakeetDownloaded, startBackgroundDownloads]);
  ...
  }, [selectedSummaryModel, hasDesktopRuntime]);
  ...
  const handleContinue = async () => {
    if (!hasDesktopRuntime) {
      toast.error('Open the desktop app to continue', {
        description: 'Model setup uses native Tauri commands and cannot run in a normal browser tab.',
      });
      return;
    }
  ...
  return (
    <OnboardingContainer
      ...
      description="You can start using MeetFlow after downloading the Transcription Engine."
      step={3}
      totalSteps={isMac ? 4 : 3}
    >
      <div className="flex flex-col items-center space-y-6">
        {!hasDesktopRuntime && (
          <div className="w-full max-w-lg rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
            <p className="font-medium">Desktop runtime required</p>
            <p className="mt-1">
              This browser preview can render the interface, but model downloads require the Tauri desktop app.
              Start MeetFlow with the desktop runtime instead of opening only <code>127.0.0.1:3118</code>.
            </p>
          </div>
        )}
