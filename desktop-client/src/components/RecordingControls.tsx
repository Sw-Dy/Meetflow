  ...
  return (
    <TooltipProvider>
      <div className="flex flex-col space-y-2">
        <div className="flex items-center space-x-3 rounded-xl border border-slate-200 bg-white/95 px-4 py-3 shadow-[0_18px_45px_rgba(15,23,42,0.16)] backdrop-blur">
          {isProcessing && !isParentProcessing ? (
            <div className="flex items-center space-x-2">
              <div className="h-5 w-5 animate-spin rounded-full border-2 border-slate-200 border-t-blue-600"></div>
              <span className="text-sm font-medium text-slate-600">Processing recording...</span>
            </div>
          ) : (
            <>
              {showPlayback ? (
                ...
                <>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          ...
                          className={`relative flex h-12 w-12 items-center justify-center rounded-lg text-white shadow-sm transition-colors ${isStarting || isProcessing || isValidatingModel ? 'bg-slate-400' : 'bg-rose-600 hover:bg-rose-700'
                            }`}
                        >
                          {isValidatingModel ? (
                        ...
                        <TooltipTrigger asChild>
                          ...
                          >
                            ...
                            {(isPausing || isResuming) && (
                              <div className="absolute -top-8 text-xs font-semibold text-slate-600">
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button
                            ...
                            className={`relative flex h-10 w-10 items-center justify-center rounded-lg text-white shadow-sm transition-colors ${isStopping || isPausing || isResuming ? 'bg-slate-400' : 'bg-rose-600 hover:bg-rose-700'
                              }`}
                          >
                            <Square size={16} />
                            {isStopping && (
                  ...
                  <div className="mx-3 flex items-center space-x-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                    {barHeights.map((height, index) => (
                      <div
                        key={index}
                        className={`w-1 rounded-full transition-all duration-200 ${isPaused ? 'bg-amber-500' : 'bg-rose-500'
                          }`}
                        style={{
