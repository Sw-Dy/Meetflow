    ...
    <div className="px-4 py-2">
                ...
                {isStreaming ? (
                  ...
                  <div className="rounded-lg border border-blue-100 bg-blue-50 px-3 py-2">
                    <div className="relative">
                      <p className="text-base leading-relaxed text-slate-800" style={{ visibility: 'hidden' }}>
                        {sizerText}
                      </p>
                      <p className="absolute left-0 top-0 text-base leading-relaxed text-slate-800">
                        {displayText}
                      </p>
                    </div>
                  </div>
      ...
      {transcripts.length === 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="mx-auto mt-16 max-w-xl rounded-xl border border-slate-200 bg-white p-8 text-center text-slate-500 shadow-sm"
        >
          {isRecording ? (
            <>
              <div className="flex items-center justify-center mb-3">
                <div className={`h-3 w-3 rounded-full ${isPaused ? 'bg-amber-500' : 'animate-pulse bg-blue-500'}`}></div>
              </div>
            ...
            <>
              <p className="text-lg font-semibold text-slate-950">Welcome to MeetFlow</p>
              <p className="mt-1 text-xs text-slate-500">No transcript captured yet.</p>
            </>
          )}
        </motion.div>
      )}
