...
export function OnboardingProvider({ children }: { children: React.ReactNode }) {
  ...
  const performAutoDetection = async () => {
    ...
    if (typeof navigator !== 'undefined' && navigator.platform?.toLowerCase().includes('mac')) {
      const homebrewDbPath = '/usr/local/var/meetflow/meeting_minutes.db';
      try {
  ...
  const completeOnboarding = async () => {
    if (!isTauriRuntime()) {
      throw new Error('MeetFlow desktop runtime is required to complete onboarding.');
    }
  ...
  const startBackgroundDownloads = async ({
    ...
    if (!isTauriRuntime()) {
      throw new Error('MeetFlow desktop runtime is required to download models.');
    }
  ...
  const retryParakeetDownload = async () => {
    if (!isTauriRuntime()) {
      throw new Error('MeetFlow desktop runtime is required to retry model downloads.');
    }
