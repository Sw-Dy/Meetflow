# GPU-accelerated development script for MeetFlow (Windows PowerShell)
# Automatically detects and runs in development mode with optimal GPU features

Write-Host "GPU-Accelerated Development Mode for MeetFlow" -ForegroundColor Blue
Write-Host ""
...
# Find desktop-client directory with package.json
if (Test-Path "package.json") {
    Write-Host "Using current directory" -ForegroundColor Cyan
} elseif (Test-Path "desktop-client\package.json") {
    Write-Host "Changing to directory: desktop-client" -ForegroundColor Cyan
    Set-Location desktop-client
} else {
    Write-Host "[ERROR] Could not find package.json" -ForegroundColor Red
    Write-Host "        Make sure you're in the project root or desktop-client directory" -ForegroundColor Red
    exit 1
}

Write-Host ""
Write-Host "Starting MeetFlow in development mode..." -ForegroundColor Blue
Write-Host ""
