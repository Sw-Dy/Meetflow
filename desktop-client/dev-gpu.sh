meetflow_demo.gif#!/bin/bash
# GPU-accelerated development script for MeetFlow
...
echo -e "${BLUE}🚀 MeetFlow GPU-Accelerated Development Mode${NC}"
echo ""
...
# Find the correct directory - we need to be in desktop client root for package commands
if [ -f "package.json" ]; then
    FRONTEND_DIR="."
elif [ -f "desktop-client/package.json" ]; then
    cd desktop-client || { echo -e "${RED}❌ Failed to change to desktop-client directory${NC}"; exit 1; }
    FRONTEND_DIR="."
else
    echo -e "${RED}❌ Could not find package.json${NC}"
    echo -e "${RED}   Make sure you're in the project root or desktop-client directory${NC}"
    exit 1
fi

echo ""
echo -e "${BLUE}📦 Starting MeetFlow in development mode...${NC}"
echo ""
...
HELPER_DIR="local-llm-helper"
if [ ! -d "$HELPER_DIR" ]; then
    # Try to find it relative to script location
    SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
    HELPER_DIR="$SCRIPT_DIR/../local-llm-helper"
fi
...
# The binary is in the workspace target directory, which is one level up from desktop-client
# if we are in desktop-client dir.
