...
function Find-ExistingDatabases {
    ...
    $commonPaths = @(
        "$env:USERPROFILE\.meetflow\meeting_minutes.db",
        "$env:USERPROFILE\Documents\meetflow\meeting_minutes.db",
    ...
    if ($env:HOMEBREW_PREFIX) {
        $commonPaths += "$env:HOMEBREW_PREFIX/Cellar/meetflow-backend/*/backend/meeting_minutes.db"
    }
