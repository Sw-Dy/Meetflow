...
show_log_exit_options() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "whisper-server\|meetflow-backend"; then
...
find_existing_databases() {
    local found_dbs=()
    local default_db_path="/opt/homebrew/Cellar/meetflow-backend/0.0.4/backend/meeting_minutes.db"
    ...
    local common_paths=(
        "/opt/homebrew/Cellar/meetflow-backend/*/backend/meeting_minutes.db"
        "$HOME/.meetflow/meeting_minutes.db"
        "$HOME/Documents/meetflow/meeting_minutes.db"
...
start_server() {
    ...
    if docker images --format "{{.Repository}}:{{.Tag}}" | grep -q "meetflow-backend:"; then
...
show_status() {
    ...
    if docker ps --format "{{.Names}}" | grep -q "meetflow-backend"; then
        app_running=true
        local app_port=$(docker port meetflow-backend 5167/tcp 2>/dev/null | cut -d: -f2)
        if [ -n "$app_port" ]; then
            log_info "Meeting App: http://localhost:$app_port"
            # Test connectivity
...
open_shell() {
    ...
    case "$service" in
        "whisper")
            ...
            ;;
        "app"|"backend")
            container_name="meetflow-backend"
            ;;
        *)
