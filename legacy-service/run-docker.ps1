...
$AppProjectName = "meetflow-backend"
$AppContainerName = "meetflow-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "meetflow-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server meetflow-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server meetflow-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "meetflow-backend"
        "backend" = "meetflow-backend"
        "meeting" = "meetflow-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "meetflow-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, meetflow-backend"
        ...
        $logArgs += @("whisper-server", "meetflow-backend")
    }
