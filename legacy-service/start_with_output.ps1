...
Write-Host "====================================="
Write-Host "MeetFlow Backend Startup"
Write-Host "====================================="
...
if (-not (Test-Path "whisper-server-package\whisper-server.exe")) {
    ...
    try {
        ...
        $apiUrl = "https://meetflow.app/releases/latest.json"
        ...
        $downloadUrl = "https://meetflow.app/releases/download/$tagName/whisper-server.exe"
        ...
        try {
            $allReleasesUrl = "https://meetflow.app/releases"
            $headers = @{"User-Agent" = "PowerShell-Script"}
            ...
            if ($releases.Count -gt 0) {
                ...
                $altDownloadUrl = "https://meetflow.app/releases/download/$latestTag/whisper-server.exe"
            ...
            Write-Host "https://meetflow.app/releases"
...
Write-Host "====================================="
Write-Host "Starting MeetFlow Backend"
Write-Host "====================================="
...
# Check if meetflow-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for meetflow-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\meetflow-frontend\meetflow-frontend.exe",
    "$env:LOCALAPPDATA\Programs\meetflow\meetflow-frontend.exe",
    "$env:ProgramFiles\meetflow-frontend\meetflow-frontend.exe",
    "${env:ProgramFiles(x86)}\meetflow-frontend\meetflow-frontend.exe",
    "$env:APPDATA\meetflow-frontend\meetflow-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("meetflow-frontend.exe", "meetflow.exe")
...
if ($frontendInstalled) {
    Write-Host "MeetFlow frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the MeetFlow frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching MeetFlow frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "MeetFlow frontend launched successfully."
        }
    }
} else {
    Write-Host "MeetFlow frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the MeetFlow frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if meetflow is now installed and launch it
