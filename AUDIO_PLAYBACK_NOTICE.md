...
When **reviewing recordings** in MeetFlow, we recommend using **computer speakers** or **wired headphones** rather than Bluetooth headphones for accurate playback.
...
1. **MeetFlow records at 48kHz** (professional audio standard)
...
- ❌ **Reviewing MeetFlow recordings** - Use wired/speakers instead
...
### This is Not a MeetFlow Bug

The issue is in **macOS's Bluetooth audio stack**, not in MeetFlow's recording engine.
...
**A**: This is a macOS/Bluetooth limitation, not a MeetFlow bug. We've correctly recorded at 48kHz.
...
**A**: Yes! MeetFlow now shows a warning when Bluetooth headphones are active during playback.
...
**Applies To**: MeetFlow v0.0.5+ on macOS
